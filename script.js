...
async function copyContent() {
    ...
    //  change timeout function incase of error
}
...
function handleCheckBoxChange() {
    ...
    else if ((numbers.checked) && (lowercase.checked) &&  (uppercase.checked)){
        checkCount=3;
    }
    else if ((symbols.checked) &&  (numbers.checked) && (lowercase.checked) && (uppercase.checked)) {
        checkCount=4;
    }
...
generateBtn.addEventListener('click', () => {
    ...
    for (let i = 0; i < arrayOfCheckedFunction.length; i++) {
        password = password + arrayOfCheckedFunction[i]();
    }
